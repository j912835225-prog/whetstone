...
| [**prose**](prose/) | **中英文都治**的成稿检查表：先定给谁看／什么场合／哪一档语气；大纲三问；删改的判据是意思不是字数；交前五挑错；评稿六看。另有两节分讲中文与英文各自的病 | 英文 |
...
**为什么有的是英文有的是中文**：这是判断，不是偷懒。两件中文的编码的就是中文叙事与古籍
翻译的工艺，译成英文本体就没了；另外七件对任何语言的材料都成立，所以走英文——其中
`prose` 一份表同时治中英文，各自的病分两节写。英文那几件的中文说明就在这张表里，看不懂
正文也知道它是干嘛的、值不值得装。
